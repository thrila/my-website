    ...
    <div >
      <Link href={`/blog/${id}`} >
        <div className='transition flex md:flex-row flex-col justify-between md:space-x-20 py-3 border-b border-zinc-400 font-thin ease-in duration-500 hover:bg-button-before hover:text-custom-background'>
          ...
          <div className='flex justify-center'><h3 className='text-left text-sm font-bold md:font-normal md:text-xl'>{formattedTitle} </h3></div>
          <div className='pr-2 w-1/8 pl-2'><p className='font-thin text-sm'>{id}</p>
          </div>
        </div>
      </Link>
    </div >
